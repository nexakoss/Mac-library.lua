...
local function CreateMinimizeButton(Window, Settings, BlurEnabled, Blurs, OpenFunction, CloseFunction, ButtonSettings)
	-- Default settings αν δεν δώσει ο χρήστης
	ButtonSettings = ButtonSettings or {}
	local icon = ButtonSettings.Icon or "rbxassetid://7733679647"
	local size = ButtonSettings.Size or 50
	local position = ButtonSettings.Position or UDim2.new(1, -60, 0, 10)
	local cornerRadius = ButtonSettings.CornerRadius or 12
	
	local MinimizeButton = Instance.new("Frame")
	MinimizeButton.Name = "MinimizeButton"
	MinimizeButton.Size = UDim2.new(0, size, 0, size)
	...
	Icon.Name = "Icon"
	Icon.Size = UDim2.new(0, size * 0.64, 0, size * 0.64)
	...
	MinimizeButton.MouseEnter:Connect(function()
		Tween(MinimizeButton, 0.2, {BackgroundColor3 = Color(Theme.Primary, 5)})
	end)
	
	MinimizeButton.MouseLeave:Connect(function()
		Tween(MinimizeButton, 0.2, {BackgroundColor3 = Theme.Primary})
	end)
	...
	MinimizeButton.InputEnded:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			if startPos and not wasDragged then
...
function Library:CreateWindow(Settings: { Title: string, Size: UDim2, Transparency: number, MinimizeKeybind: Enum.KeyCode?, Blurring: boolean, Theme: string })
	...
	MinimizeButton = CreateMinimizeButton(Window, Settings, BlurEnabled, Blurs, Open, Close)
	...
	function Options:AddSlider(Settings: { Title: string, Description: string, MaxValue: number, AllowDecimals: boolean, DecimalAmount: number, Tab: Instance, Callback: any }) 
		...
		local SetNumber = function(Number)
			if Settings.AllowDecimals then
				...
				Number = math.floor(Number * Power + 0.5)
				else
